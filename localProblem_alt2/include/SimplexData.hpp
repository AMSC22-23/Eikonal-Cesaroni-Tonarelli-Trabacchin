...
template<std::size_t PHDIM, int N>
  struct SimplexData
  {
	using AnisotropyM=typename Eikonal_traits<PHDIM, N>::AnisotropyM;
	using MMatrix=typename Eikonal_traits<PHDIM, N>::MMatrix;
	using Point=typename Eikonal_traits<PHDIM, N>::Point;
  ...
  private:
	  void setup(AnisotropyM const &M)
	  {
		  ...
		  if constexpr(N == 4)
    		{
			  E.col(1)=points[N - 2]-points[1]; //e23
    		}
